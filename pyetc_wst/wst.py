            ...
            'history': [
                {
                    ...
                    'changes': [
                        ...
                        'Fixed cross-optimisation consistency in time_from_source_ifs(compute="best") when COADD_XY="best": the optimal spatial aperture is now jointly solved with DIT+NDIT via an iterative feedback loop (≤5 iterations). After each analytical DIT+NDIT solve the coadd is re-optimised with _resolve_best_coadd_ifs until convergence, ensuring the returned DIT is self-consistent with the aperture used. Returns dit_sat (saturation-limited DIT most efficient for readout noise), ndit_raw (fractional NDIT before ceiling), and ima_coadd (frozen integer aperture). time_from_source_mos(compute="best") also returns dit_sat and ndit_raw.',
                        'Fixed PSF parity inconsistency in the best-mode coadd-DIT feedback loop: when the feedback loop changes the optimal coadd from even to odd (or vice versa), the PSF image (selected_image) is now recomputed with the updated uneven flag. This ensures the source flux fractions used in the analytical SNR formula match those computed by snr_from_source_ifs, eliminating an ~18% SNR discrepancy that arose when the initial and final coadds had different parities.',
                        'Web interface — best+window cross-optimisation: adopted a 2-step approach that eliminates the NDIT≈0 failure seen when a plain DIT-optimal exposure is too long for a target window SNR. Step 1 calls time_from_source(compute="best") to get DIT_sat; step 2 calls time_from_source(compute="ndit") with DIT fixed at DIT_sat to find the NDIT that reaches the window target. For very bright sources where ndit_raw<1 (NDIT=1 over-exposes even at DIT_sat), NDIT is fixed to 1 and DIT is iterated instead. The auto-optimised coadd is also frozen before the final snr_from_source display call so the displayed SNR is consistent with the time computation.',
                        'Fixed saturation flag in snr_from_source_ifs and snr_from_source_mos: the per-pixel peak counts are now divided by NDIT before comparison with threshold_sat (50 000 e⁻). nph_source and nph_sky include the full DIT×NDIT integrated counts; dividing by NDIT correctly tests whether any single exposure saturates a pixel.',
                        'Fixed saturation line in plot and peak_counts in API (IFS): the secondary-axis peak-counts curve and the API peak_counts field now show per-single-DIT counts (nph_source_peak + nph_sky_peak divided by NDIT), keeping them directly comparable to the 50 000 e⁻ saturation threshold line.',
                        'Added dit_at_min_floor flag in time_from_source_window(compute="dit"): set to True when the converged DIT reaches the 0.1 s instrument minimum floor. The web interface uses this flag to emit a warning that the source is too bright to achieve the target SNR at the given wavelength.',
                    ],
                },
                {
                    'version': '1.4',
