...
- **`time_from_source_window` convergence improved**: max iterations raised to 20; convergence criterion changed to SNR-relative tolerance `1e-4` (0.01%); NDIT sub-1 case exits after one exact analytical step; no artificial floor on `param_val` during iteration. **NDIT bracket detection** added: once consecutive integers N and N+1 have both been evaluated and straddle the target SNR, the loop exits immediately (3 iterations instead of 20 in the typical bracketing case). **DIT secant acceleration** added: from iteration 2 onward, the next DIT is computed by linear interpolation between the two most recent (DIT, SNR) points rather than the slow multiplicative update, reducing typical DIT convergence from 20 to ~4 iterations.
